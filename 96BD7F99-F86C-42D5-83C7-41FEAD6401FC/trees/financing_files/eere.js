//EERE GA Tracking Code
//Version 2.9.3


/* Tell jshint to ignore the escaping in our regex strings */
/* jshint -W044, quotmark: false, browser: true, jquery: true */
"use strict";
var GATrackerObject = {
  globalRollupID: "UA-24103967-1", //EERE total rollup ID
  enableGRTracking: true,
  profileID: "-",
  dataSource: "",
  subsite: "",
  ssTracker: new Object(),
  grTracker: new Object(),
  cookieDomain: document.location.hostname,
  grCookieDomain: "",
  onsiteSearchHost: "search.nrel.gov",
  trackOnsiteSearch: false,
  onsiteSearchTerm: "",
  modPageURI: "",
  universal: true,
  crossSiteDomains: ["energy.gov", "annualmeritreview.energy.gov", "eereblogs.energy.gov", "www.afdc.energy.gov", "www.energycodes.gov", "www.energyempowers.gov", "www.solardecathlon.gov", "eeremultimedia.energy.gov", "federalfleets.energy.gov", "solarhighpen.energy.gov", "basc.pnnl.gov", "bbnp.pnnl.gov", "betterbuildingssolutioncenter.energy.gov", "waveenergyprize.org","nrel.github.io"],
  downloadDomains: ["energy.gov", "nrel.github.io", "eere.energy.gov", "annualmeritreview.energy.gov", "eereblogs.energy.gov", "www.afdc.energy.gov", "www.energyempowers.gov", "www.energycodes.gov", "www.solardecathlon.gov", "federalfleets.energy.gov", "solarhighpen.energy.gov", "eeremultimedia.energy.gov", "www.hydrogen.energy.gov", "www.energysavers.gov", "eere-intranet2.ee.doe.gov", "eeftp.ee.doe.gov", "basc.pnnl.gov", "bbnp.pnnl.gov", "nrel.gov", "betterbuildingssolutioncenter.energy.gov", "waveenergyprize.org"],
  downloadFileExtensions: ["pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "zip", "exe", "dat", "wvx", "wax", "asf", "asx", "wma", "mp4", "mp3", "m3u", "mpg", "swf", "avi", "wmv", "mov", "dmg", "jpg"],
  dataSources: {
    "Bioenergy": {
      id: "UA-25110701-1",
      title: "EERE Bioenergy",
      universal: true
    },
    "BetterBuildings": {
      id: "UA-55403375-1",
      title: "EERE Better Buildings",
      universal: true
    },
    "Buildings": {
      id: "UA-25064180-1",
      title: "EERE Buildings",
      universal: true
    },
    "FEMP": {
      id: "UA-25108260-1",
      title: "EERE FEMP",
      universal: true
    },
    "FEMP-Sustainability": {
      id: "UA-25108260-2",
      title: "Sustainability Performance Office",
      universal: true
    },
    "FCT": {
      id: "UA-25108261-1",
      title: "EERE Fuel Cells",
      universal: true
    },
    "FCT-AnnualMeritReview": {
      id: "UA-25108261-2",
      title: "Annual Merit Review",
      universal: true
    },
    "Geothermal": {
      id: "UA-25109423-1",
      title: "EERE Geothermal",
      universal: true
    },
    "Manufacturing": {
      id: "UA-25110219-1",
      title: "EERE Manufacturing",
      universal: true
    },
    "Solar": {
      id: "UA-25110720-1",
      title: "EERE Solar",
      universal: true
    },
    "Vehicles": {
      id: "UA-25110724-1",
      title: "EERE Vehicles",
      universal: true
    },
    "Weatherization": {
      id: "UA-25108667-1",
      title: "EERE Weatherization",
      universal: true
    },
    "Wind-and-Water": {
      id: "UA-25109557-1",
      title: "EERE Wind & Water",
      universal: true
    },
    "Energy Savers": {
      id: "UA-25066145-1",
      title: "Energy Savers",
      universal: true
    },
    "Solar Decathlon": {
      id: "UA-24144971-1",
      title: "Solar Decathlon",
      universal: true
    },
    "DOE Hydrogen": {
      id: "UA-25066918-1",
      title: "DOE Hydrogen",
      universal: true
    },
    "Corporate": {
      id: "UA-25108607-1",
      title: "EERE Corporate",
      universal: true
    },
    "Corporate DEV": {
      id: "UA-25108607-5",
      title: "EERE Corporate DEV",
      universal: true
    },
    "Corporate-Intranet": {
      id: "UA-25108607-2",
      title: "EERE Intranet",
      universal: true
    },
    "Corporate-Grid Workshop": {
      id: "UA-25108607-3",
      title: "DOE Grid Integration Workshop",
      universal: true
    },
    "TEST" : {
      id : "UA-52836309-1",
      title : "EERE TEST",
      universal: true
    },
    "Wave Energy Prize": {
      id: "UA-58830142-1",
      title: "Wave Energy Prize",
      universal: true
    },
    "Wind Vision": {
      id: "UA-65221038-1",
      title: "Wind Vision",
      universal: true
    }

  },
  sites: [
    //URL RegEx match, dataSource name (Property), View, include in EERE Total
    ["(www\.)?energy\.gov/eere/wind/maps/wind-vision", "Wind Vision", "", true],
    ["(www\.)?energy\.gov/eere/eveverywhere", "Vehicles", "", true],
    ["www(1|3)?\.eere\.energy\.gov/bioenergy/", "Bioenergy", "", true],
    ["www\.eereblogs\.energy\.gov/bioenergy/", "Bioenergy", "", true],
    ["www1\.eere\.energy\.gov/buildings/betterbuildings/", "BetterBuildings", "", true],
    ["www4\.eere\.energy\.gov/(workforce|alliance|challenge)/", "BetterBuildings", "", true],
    ["(www|apps)(1|4)?\.eere\.energy\.gov/buildings/", "Buildings", "", true],
    ["www\.eereblogs\.energy\.gov/buildingenvelope/", "Buildings", "", true],
    ["www\.energycodes\.gov/", "Buildings", "", true],
    ["(basc|bbnp)\.pnnl\.gov/", "Buildings", "", true],
    ["betterbuildingssolutioncenter\.energy\.gov", "BetterBuildings", "", true],
    ["rpsc\.energy\.gov/", "BetterBuildings", "", true],
    ["www5\.eere\.energy\.gov/office_eere/(current\_budget|program\_budget\_formulation)", "Corporate", "", true],
    ["(www|apps)(1|3|4)?\.eere\.energy\.gov/femp/", "FEMP", "", true],
    ["federalfleets\.energy\.gov/", "FEMP", "Fleet", true],
    ["epact\.energy\.gov", "Vehicles", "", true],
    ["energy\.gov/eere/spo/", "FEMP-Sustainability", "", true],
    ["www1?\.eere\.energy\.gov/team/", "FEMP-Team", "", true],
    ["www1?\.eere\.energy\.gov/hydrogenandfuelcells/", "FCT", "", true],
    ["www1?\.annualmeritreview\.energy\.gov/", "FCT-AnnualMeritReview", "", true],
    ["www(1|4)?\.eere\.energy\.gov/geothermal/", "Geothermal", "", true],
    ["(www|apps)(1|4)?\.eere\.energy\.gov/(manufacturing|industry|inventions)/", "Manufacturing", "", true],
    ["(www|apps)(1|4)?\.eere(multimedia)?\.energy\.gov/(solar)?(challenge)?/", "Solar", "", true],
    ["solarhighpen\.energy\.gov", "Solar", "", true],
    ["www\.afdc\.energy\.gov/cleancities", "Vehicles", "", true],
    ["www\.afdc\.energy\.gov/workplace-charging/", "Vehicles", "", true],
    ["www\.eereblogs\.energy\.gov/cleancities/", "Vehicles", "", true],
    ["www\.eereblogs\.energy\.gov/lessonslearned/", "Corporate", "Lessons Learned", true],
    ["(www|apps)(1|4)?\.eere\.energy\.gov/(wip|repi|tribalenergy|weatherization|state_energy_program)/", "Weatherization", "", true],
    ["(www|apps)1?\.eere\.energy\.gov/windandhydro/", "Wind-and-Water", "", true],
    ["(www|apps)(1|4|2)?\.eere\.energy\.gov/(wind|water)/", "Wind-and-Water", "", true],
    ["energy\.gov/energysaver/", "Energy Savers", "", false],
    ["(www\.)?energysavers\.gov/", "Energy Savers", "", false],
    ["www\.solardecathlon\.gov/", "Solar Decathlon", "", true],
    ["www\.hydrogen\.energy\.gov/", "DOE Hydrogen", "", false],
    ["eere-intranet2\.ee\.doe\.gov/", "Corporate-Intranet", "EERE Intranet", false],
    ["apps1\.eere\.energy\.gov/grid_integration_workshop/", "Corporate-Grid Workshop", "DOE Grid Integration Workshop", false],
    ["www1?\.eere\.energy\.gov/analysis/", "Corporate", "Energy Analysis", true],
    ["(www\.)?energy\.gov/eere/analysis/program-evaluation", "Corporate", "Energy Analysis", true],
    ["(www\.)?energy\.gov/eere/about-us/policy-and-analysis-team", "Corporate", "Energy Analysis", true],
    ["www4?\.eere\.energy\.gov/workforce/", "Corporate", "Better Buildings Workforce", true],
    ["(www\.)?biomassboard\.gov/", "Corporate", "Biomass Board", false],
    ["www1?\.eere\.energy\.gov/ba/pba/", "Corporate", "Planning, Budget, and Analysis", true],
    ["www1?\.eere\.energy\.gov/ba/", "Corporate", "Business Administration", true],
    ["www1?\.eere\.energy\.gov/communicationstandards/", "Corporate", "Communication Standards", true],
    ["(www|apps)1?\.eere\.energy\.gov/deployment/", "Corporate", "Deployment", true],
    ["www4?\.eere\.energy\.gov/disastercenter/", "Corporate", "Disaster and Energy Information", true],
    ["www5\.eere\.energy\.gov/eereatwork/", "Corporate", "EERE at Work", true],
    ["www1?\.eere\.energy\.gov/basics/", "Corporate", "Energy Basics", true],
    ["www1?\.eere\.energy\.gov/calculators/", "Corporate", "Energy Calculators", true],
    ["www1?\.eere\.energy\.gov/education/postdoctoral/", "Corporate", "Energy Education Postdoctoral Research Awards", true],
    ["www1?\.eere\.energy\.gov/education/lessonplans/", "Corporate", "Energy Education K12 Lesson Plans and Activities", true],
    ["www\.energyempowers\.gov/", "Corporate", "Energy Empowers", true],
    ["techportal\.eere\.energy\.gov/", "Corporate", "Energy Innovation Portal", true],
    ["(www\.)?energy\.gov/eere/funding/funding-opportunities", "Corporate", "Financial Opportunities", true],
    ["www4\.eere\.energy\.gov/fundingportal/", "Corporate", "Funding Portals", true],
    ["www4\.eere\.energy\.gov/svip/apply/", "Corporate", "About Us", true],
    ["apps3\.eere\.energy\.gov/greenpower/", "Corporate", "Green Power", true],
    ["www1?\.eere\.energy\.gov/informationcenter/", "Corporate", "Information Center", true],
    ["www1?\.eere\.energy\.gov/international/", "Corporate", "International Activities", true],
    ["www1?\.eere\.energy\.gov/kids/", "Corporate", "Kids", true],
    ["www1?\.eere\.energy\.gov/multimedia/", "Corporate", "Multimedia", true],
    ["apps1\.eere\.energy\.gov/news/", "Corporate", "News", true],
    ["www1?\.eere\.energy\.gov/office_eere/", "Corporate", "Office of EERE", true],
    ["www1?\.eere\.energy\.gov/fieldoperations/", "Corporate", "Office of Field Operations", true],
    ["www1?\.eere\.energy\.gov/pir/", "Corporate", "Program Plans, Implementation, and Results", true],
    ["www1?\.eere\.energy\.gov/library/", "Corporate", "Publication and Product Library", true],
    ["www1?\.eere\.energy\.gov/socialmedia/", "Corporate", "Social Media", true],
    ["apps1\.eere\.energy\.gov/states/", "Corporate", "State Activities and Partnerships", true],
    ["www4?\.eere\.energy\.gov/seeaction/", "Corporate", "State and Local Energy Efficiency Action Network", true],
    ["apps1\.eere\.energy\.gov/sled/", "Corporate", "State and Local Energy Data", true],
    ["(www\.)?energy\.gov/eere/success-stories/", "Corporate", "EERE Success Stories", true],
    ["apps1\.eere\.energy\.gov/survey/", "Corporate", "Survey", true],
    ["www1?\.eere\.energy\.gov/topics/", "Corporate", "Topic Landing Pages", true],
    ["www\.eere\.energy\.gov/(index\.html)?", "Corporate", "Home Page", true],
    ["(www\.)?energy\.gov/eere/bioenergy", "Bioenergy", "", true],
    ["(www\.)?energy\.gov/eere/buildings", "Buildings", "", true],
    ["(www\.)?energy\.gov/eere/better-buildings-neighborhood-program", "BetterBuildings", "", true],
    ["(www\.)?energy\.gov/eere/better-buildings-residential-network", "BetterBuildings", "", true],
    ["(www\.)?energy\.gov/eere/ssl", "Buildings", "", true],
    ["(www\.)?energy\.gov/eere/fuelcells", "FCT", "", true],
    ["(www\.)?energy\.gov/eere/femp", "FEMP", "", true],
    ["(www\.)?energy\.gov/eere/geothermal", "Geothermal", "", true],
    ["(www\.)?energy\.gov/eere/forge", "Geothermal", "", true],
    ["(www\.)?energy\.gov/eere/amo", "Manufacturing", "", true],
    ["(www\.)?energy\.gov/eere/sunshot", "Solar", "", true],
    ["(www\.)?energy\.gov/eere/solarpoweringamerica", "Solar", "", true],
    ["(www\.)?energy\.gov/eere/vehicles", "Vehicles", "", true],
    ["(www\.)?energy\.gov/eere/(wipo|slsc)", "Weatherization", "", true],
    ["(www\.)?energy\.gov/eere/wind", "Wind-and-Water", "", true],
    ["(www\.)?energy\.gov/eere/water", "Wind-and-Water", "", true],
    ["(www\.)?energy\.gov/eere/collegiatewindcompetition", "Wind-and-Water", "", true],
    ["(www\.)?energy\.gov/eere/cemi", "Corporate", "Clean Energy Manufacturing Initiative", true],
    ["(www\.)?energy\.gov/eere/education", "Corporate", "Energy Education and Workforce Development", true],
    ["(www\.)?energy\.gov/eere/", "Corporate", "EERE Pages on energy.gov", true],
    ["dev\.cms\.doe\.gov", "Corporate DEV", "DEV - EERE Pages on energy.gov", false],
    ["stage\.cms\.doe\.gov", "Corporate DEV", "DEV - EERE Pages on energy.gov", false],
    ["commdev2\.nrel\.gov/drrl/", "TEST", "Disaster Recovery & Energy Information Center", false],
    ["eeredev\.nrel\.gov/islandsplaybook", "TEST", "", false],
    ["apps1\.eere\.energy\.gov/islandsplaybook", "Corporate", "ETI Islands Playbook", true],
    ["waveenergyprize\.org", "Wave Energy Prize", "", true],
    ["(www\.)?energy\.gov/eere/wind/maps/wind-vision", "Wind Vision", "", true]
  ],
  //method to set GA IDs
  setIDs: function () {
    var testURL = document.location.href,
        i, matchFound = false;
    this.cookieDomain = document.location.hostname;
    this.grCookieDomain = (this.cookieDomain.split(".").length > 2) ? this.cookieDomain.substr(this.cookieDomain.indexOf(".")) : "." + this.cookieDomain;
    if( window.location.pathname.toLowerCase().indexOf("/search/site") > -1 ){
      var recentIndex = this.getCookie("_gaRecentSiteIndex");
      if( recentIndex !== "" && !isNaN(recentIndex) ){
        i = parseInt(recentIndex);
        var recentUri = this.getCookie("_gaRecentSiteUri");
        this.modPageURI = (recentUri !== "") ? recentUri + window.location.pathname.toLowerCase() : "";
        this.trackOnsiteSearch = true;
        matchFound = true;
      } else {
        for (i = 0; i < this.sites.length; i++) {
          var pattern = new RegExp("^http(s)?://" + this.sites[i][0] + ".*$");
          if (pattern.test(testURL)) {
            matchFound = true;
            this.setCookie("_gaRecentSiteIndex",i,this.grCookieDomain);
            this.setCookie("_gaRecentSiteUri",window.location.pathname.toLowerCase(),this.grCookieDomain);
            break;
          } else {
            this.profileID = "-";
          }
        }
      }
    } else {
      for (i = 0; i < this.sites.length; i++) {
        var pattern = new RegExp("^http(s)?://" + this.sites[i][0] + ".*$");
        if (pattern.test(testURL)) {
          matchFound = true;
          this.setCookie("_gaRecentSiteIndex",i,this.grCookieDomain);
          this.setCookie("_gaRecentSiteUri",window.location.pathname.toLowerCase(),this.grCookieDomain);
          break;
        } else {
          this.profileID = "-";
        }
      }
    }
    if( matchFound ){
      this.profileID = this.dataSources[this.sites[i][1]].id;
      this.dataSource = this.sites[i][1];
      if (this.dataSource === "Corporate") {
        this.subsite = this.sites[i][2];
      }
      if (this.sites[i][3]) {
        this.enableGRTracking = true;
      } else {
        this.enableGRTracking = false;
      }
      if (this.dataSources[this.sites[i][1]].universal) {
        this.universal = true;
      }
    }
  },
  //method to track onsite search
  setOnsiteSearch: function () {
    var referrerURL = document.referrer;
    var referrerHost = referrerURL.split("/")[2];
    if (referrerHost === this.onsiteSearchHost) {
      var queryString = referrerURL.split("?")[1].split("&");
      for (var i = 0; i < queryString.length; i++) {
        var keyValue = queryString[i].split("=");
        if (keyValue[0] === "qt") {
          this.trackOnsiteSearch = true;
          this.onsiteSearchTerm = keyValue[1];
          this.modPageURI = "osk=" + keyValue[1].toLowerCase() + "&osc=" + document.location.hostname.toLowerCase();
          if (document.location.pathname.indexOf("?") > 1) {
            this.modPageURI = document.location.pathname + "&" + this.modPageURI;
          } else {
            this.modPageURI = document.location.pathname + "?" + this.modPageURI;
          }
          break;
        }
      }
    }
  },
  // method to track onsite search as event
  trackOnsiteSearchEvent: function(){
    var lcPath = window.location.pathname.toLowerCase();
    if( lcPath.indexOf("/search/site") > -1 ){
      var lcPathParts = lcPath.split('/'),
          searchPhrase = lcPathParts[3]||"",
          resultsSpan = this.getElementsByClassName("search-no-results-statistics",document,false,"SPAN");
      this.trackEvent("Onsite Search",(resultsSpan.length === 0) ? "Results" : "No Results",decodeURIComponent(searchPhrase));
      var numSearches = parseInt(this.getCookie("_gaSearchPerformed")||"0");
      if( document.location.search.indexOf("page=") === -1 ){
        this.setCookie("_gaSearchPerformed",(++numSearches));
      }
      this.trackOnsiteSearchInteraction(searchPhrase,numSearches);
      this.setCookie("_gaSearchPhrase",searchPhrase);
    }
  },
  // method to track interaction with onsite search results
  trackOnsiteSearchInteraction: function(searchPhrase,numSearches){
    if( numSearches > 1 && searchPhrase !== this.getCookie("_gaSearchPhrase") ){
      this.trackEvent("Onsite Search","Not Found",decodeURIComponent(this.getCookie("_gaSearchPhrase")));
    }
    this._onsiteSearchInteractionInterval = setInterval(function(){
      var resultsLists = GATrackerObject.getElementsByClassName("search-results",document,true,"UL");
      if( resultsLists.length > 0 ){
        clearInterval(GATrackerObject._onsiteSearchInteractionInterval);
        var resultsLinks = (resultsLists.length > 0) ? resultsLists[0].getElementsByTagName("A") : [];
        for( var i = 0, numLinks = resultsLinks.length; i < numLinks; i++ ){
          var currentLink = resultsLinks[i];
          currentLink._gaCurrentPhrase = searchPhrase;
          currentLink.oldonclick = currentLink.onclick||function(){};
          currentLink.onclick = function(evt){
            GATrackerObject.trackEvent("Onsite Search","Found",decodeURIComponent(this._gaCurrentPhrase||""));
            GATrackerObject.setCookie("_gaSearchPerformed","0");
            return this.oldonclick(evt);
          };
        }
      }
    },200);
  },
  //method to track downloads
  trackDownload: function (linkObject) {
    var linkURL = linkObject.href;
    var linkURLChunks = linkURL.split("//")[1].split("/");
    var fileName = linkURLChunks[linkURLChunks.length - 1].split("?")[0].split("#")[0];
    var fileExtension = fileName.split(".").reverse()[0];
    GATrackerObject.trackEvent(fileExtension, fileName, linkURL);
    /*
    _gaq.push(function () {
      GATrackerObject.ssTracker._trackEvent(fileExtension, fileName, linkURL);
      if (GATrackerObject.enableGRTracking) {
        GATrackerObject.grTracker._trackEvent(fileExtension, fileName, linkURL);
      }
    });
    */
  },
  trackOffsiteLink: function (linkObject, type) {
    var linkURL = linkObject.href.toLowerCase(),
        linkURLChunks = linkURL.split("//")[1].split("/"),
        linkURLHost = linkURLChunks[0],
        linkWithoutProtocol = linkURLChunks.join('/');
    GATrackerObject.trackEvent(type+" Link", linkWithoutProtocol, linkURLHost+":"+document.domain.toLowerCase()+window.location.pathname.toLowerCase()+document.location.search.toLowerCase());
  },
  //method to enable download and crossite domain tracking
  trackLinks: function () {
    var pageLinks = document.links;
    for (var i = 0; i < pageLinks.length; i++) {
      if (pageLinks[i].href.indexOf("//") > -1) {
        var linkURLChunks = pageLinks[i].href.split("//")[1].split("/");
        var linkURLHost = linkURLChunks[0];
        var methodApplied = false;
        var downloadFlag = false;
        var fileName = linkURLChunks[linkURLChunks.length - 1].split("?")[0].split("#")[0];
        if (fileName.length > 0 && fileName.indexOf(".") > 1) {
          var fileExtension = fileName.split(".").reverse()[0];
          for (var n = 0; n < GATrackerObject.downloadFileExtensions.length; n++) {
            if (fileExtension === GATrackerObject.downloadFileExtensions[n]) {
              for (var m = 0; m < GATrackerObject.downloadDomains.length; m++) {
                if (linkURLHost.indexOf(GATrackerObject.downloadDomains[m]) > -1) {
                  downloadFlag = true;
                  break;
                }
              }
              if (downloadFlag) {
                pageLinks[i].onclick = function () {
                  GATrackerObject.trackDownload(this);
                };
                methodApplied = true;
                break;
              }
            }
          }
        }
        if (!methodApplied && GATrackerObject.enableGRTracking) {
          if (linkURLHost !== document.location.hostname) {
            for (var j = 0; j < GATrackerObject.crossSiteDomains.length; j++) {
              if (linkURLHost === GATrackerObject.crossSiteDomains[j]) {
                pageLinks[i].onclick = function () {
                  try{
                    GATrackerObject.trackOffsiteLink(this, "Cross-Domain");
                  }catch(e){}
                  if( GATrackerObject.universal ){
                    ga('linker:decorate', this);
                  } else {
                    GATrackerObject.grTracker._link(this.href);
                    return false;
                  }
                };
                methodApplied = true;
                break;
              }
            }
          }
        }
        if (!methodApplied) {
          var lcLinkURLHost = linkURLHost.toLowerCase(),
              lcDomain = document.domain.toLowerCase();
          if (lcLinkURLHost.indexOf(lcDomain) === -1 && lcDomain.indexOf(lcLinkURLHost) === -1) {
            pageLinks[i].onclick = function () {
              try{
                GATrackerObject.trackOffsiteLink(this, "Exit");
              }catch(e){}
            };
          }
        }
      }
    }
  },
  // method to encapsulate custom variable functionality
  setCustomVar: function(index, key, value, scope){
    if( this.universal ){
      this.universalData = this.universalData || {};
      this.universalData["dimension"+index] = value;
      this.universalData["dimension1"+index] = key;
    } else {
      GATrackerObject.ssTracker._setCustomVar(index, key, value, scope);
    }
  },
  trackEvent: function(category, action, label, value, noninteractive){
    if( this.universal ){
      var eventData = {};
      if( noninteractive ) {
        eventData.nonInteraction = 1;
      }
      for( var key in this.universalData ){
        eventData[key] = this.universalData[key];
      }
      if( this.trackOnsiteSearch ){
        eventData.page = this.modPageURI;
      }
      ga('ssTracker.send', 'event', category, action, label, value, eventData);
      if (GATrackerObject.enableGRTracking) {
        ga('grTracker.send', 'event', category, action, label, value, eventData);
      }
    } else {
      _gaq.push(function () {
        GATrackerObject.ssTracker._trackEvent(category, action, label, value, noninteractive);
        if (GATrackerObject.enableGRTracking) {
          GATrackerObject.grTracker._trackEvent(category, action, label, value, noninteractive);
        }
      });
    }
  },
  trackSocial: function(network, action, target, pagePath){
    if( this.universal ){
      var pathObj = (typeof pagePath === "string") ? {'page': pagePath} : {};
      ga('ssTracker.send', 'social', network, action, target, pathObj);
      if (GATrackerObject.enableGRTracking) {
        ga('grTracker.send', 'social', network, action, target, pathObj);
      }
    } else {
      GATrackerObject.ssTracker._trackSocial(network, action, target, pagePath);
      if (GATrackerObject.enableGRTracking) {
        GATrackerObject.grTracker._trackSocial(network, action, target, pagePath);
      }
    }
  },
  setCookie: function(c_name,value,domain,exdays){
    var expiry = "",
        domain = domain || this.grCookieDomain;
    if( exdays ){
      var exdate = new Date();
      exdate.setDate(exdate.getDate() + exdays);
      expiry = "; expires="+exdate.toUTCString();
    }
    var c_value = escape(value) + "; path=/; domain=" + domain + expiry;
    document.cookie = c_name + "=" + c_value;
  },
  getCookie: function(c_name) {
    var c_value = document.cookie;
    var c_start = c_value.indexOf(" " + c_name + "=");
    if (c_start == -1) {
      c_start = c_value.indexOf(c_name + "=");
    }
    if (c_start == -1) {
      c_value = "";
    } else {
      c_start = c_value.indexOf("=", c_start) + 1;
      var c_end = c_value.indexOf(";", c_start);
      if (c_end == -1) {
        c_end = c_value.length;
      }
      c_value = unescape(c_value.substring(c_start, c_end));
    }
    return c_value;
  },
  getElementsByClassName: function(className,root,partial,tagName){
    root = root || document;
    className = className || '';
    partial = partial || false;
    tagName = tagName || "*";
    var elements = root.getElementsByTagName(tagName);
    var retElements = [],
        needle = partial ? new RegExp(className) : new RegExp('^' + className + '$');
    for (var i = 0; i < elements.length; i++) {
      var tempClass = elements[i].className||"";
      if (tempClass.match(needle)) {
        retElements.push(elements[i]);
      }
    }
    return retElements;
  }
};
GATrackerObject.setIDs();
if (GATrackerObject.universal) {
  // Universal Analytics tracking
  (function (i, s, o, g, r, a, m) {
    i['GoogleAnalyticsObject'] = r;
    i[r] = i[r] || function () {
      (i[r].q = i[r].q || []).push(arguments);
    }, i[r].l = 1 * new Date();
    a = s.createElement(o),
      m = s.getElementsByTagName(o)[0];
    a.async = 1;
    a.src = g;
    m.parentNode.insertBefore(a, m);
  })(window, document, 'script', '//www.google-analytics.com/analytics.js', 'ga');
  ga(function () {
    GATrackerObject.setOnsiteSearch();
    if (GATrackerObject.profileID !== "-") {
      ga('create', GATrackerObject.profileID, {
        'name': 'ssTracker',
        'cookieDomain': GATrackerObject.cookieDomain,
        'cookieName': 'ssGA'
      });
      var pageInfo = (GATrackerObject.trackOnsiteSearch) ? {
        'page': GATrackerObject.modPageURI
      } : {};
      ga('ssTracker.send', 'pageview', pageInfo);
    }
    if (GATrackerObject.enableGRTracking) {
      ga('create', GATrackerObject.globalRollupID, {
        'name': 'grTracker',
        'cookieDomain': GATrackerObject.cookieDomain,
        'cookieName': 'grGA',
        'allowLinker': true
      });
      var pageInfo = (GATrackerObject.trackOnsiteSearch) ? {
        'page': GATrackerObject.modPageURI
      } : {};
      ga('grTracker.send', 'pageview', pageInfo);
    }
    GATrackerObject.trackOnsiteSearchEvent();
    if (typeof jQuery !== 'undefined') {
      jQuery(document).ready(function () {
        GATrackerObject.trackLinks();
      });
    } else {
      ga('create', 'UA-24103967-2', {
        'name': 'jqTracker',
        'cookieName': 'jqGA'
      });
      ga('jqTracker.send', 'pageview');
      GATrackerObject.trackLinks();
    }
  });
} else {
  // Classic Analytics tracking
  var _gaq = _gaq || [];
  _gaq.push(function () {
    GATrackerObject.setOnsiteSearch();
    if (GATrackerObject.profileID !== "-") {
      GATrackerObject.ssTracker = _gat._createTracker(GATrackerObject.profileID, 't1');
      GATrackerObject.ssTracker._setDomainName(GATrackerObject.cookieDomain);
      if (GATrackerObject.trackOnsiteSearch) {
        GATrackerObject.ssTracker._trackPageview(GATrackerObject.modPageURI);
      } else {
        GATrackerObject.ssTracker._trackPageview();
      }
    }
    if (GATrackerObject.enableGRTracking) {
      GATrackerObject.grTracker = _gat._createTracker(GATrackerObject.globalRollupID, 't2');
      GATrackerObject.grTracker._setAllowHash(false);
      GATrackerObject.grTracker._setAllowLinker(true);
      GATrackerObject.grTracker._setDomainName(GATrackerObject.grCookieDomain);
      if (GATrackerObject.trackOnsiteSearch) {
        GATrackerObject.grTracker._trackPageview(GATrackerObject.modPageURI);
      } else {
        GATrackerObject.grTracker._trackPageview();
      }
    }
    if (typeof jQuery !== 'undefined') {
      jQuery(document).ready(function () {
        GATrackerObject.trackLinks();
      });
    } else {
      var jqTracker = _gat._createTracker('UA-24103967-2');
      jqTracker._trackPageview();
      GATrackerObject.trackLinks();
    }
  });
  (function () {
    var ga = document.createElement('script');
    ga.type = 'text/javascript';
    ga.async = true;
    ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js';
    var s = document.getElementsByTagName('script')[0];
    s.parentNode.insertBefore(ga, s);
  })();
}

//AddThis Share link tracking
window.shareLinkCounter = 0;
window.shareLinkId = setInterval(function(){
  try{
    window.shareLinkCounter++;
    jQuery(document).ready(function () {
      var addThisLinks = jQuery("#share li a");
      window._cntBtn = null;
      for (var i = 0; addThisLinks.length > i; i++) {
        addThisLinks[i].onclick = function () {
          window.addThisLastClickedLink = this.className;
          if (!window._cntBtn) {
            window._cntBtn = document.getElementById("continueExternal");
            window._cntBtn.onmousedown = function () {
              GATrackerObject.trackEvent("AddThis", "Share Links", window.addThisLastClickedLink);
              GATrackerObject.trackSocial(window.addThisLastClickedLink, "Share");
              /*GATrackerObject.ssTracker._trackEvent("AddThis", "Share Links", window.addThisLastClickedLink);
              GATrackerObject.ssTracker._trackSocial(window.addThisLastClickedLink, "Share");
              if (GATrackerObject.enableGRTracking) {
                GATrackerObject.grTracker._trackEvent("AddThis", "Share Links", window.addThisLastClickedLink);
                GATrackerObject.grTracker._trackSocial(window.addThisLastClickedLink, "Share");
              }*/
            };
          }
        };
      }
    });
    clearInterval(window.shareLinkId);
  } catch(e){
    if( window.shareLinkCounter > 100 ){
      clearInterval(window.shareLinkId);
    }
  }
}, 100);